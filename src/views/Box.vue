<template>
    <div :class="['box', sizeClass]">
        ...
        <ui-icon-button class="setting" icon="settings"  @click="showSetting" />
        <div class="settingBox" v-if="settingVisible">
            <div class="widgetList">
                <div class="widget iframeBox" v-for="plugin, index in plugins">
                    <div class="name">{{ plugin.name }}</div>
                    <textarea class="textarea" v-model="plugin.url" rows="3"></textarea>
                    <!-- <iframe class="iframe" :src="plugin.url" frameborder="0"></iframe> -->
                    <ui-raised-button primary @click="change(index)">切换</ui-raised-button>
                </div>
            </div>
            <ui-icon-button class="setting" icon="check"  @click="closeSetting" />
        </div>
        <div class="selectBox" v-if="selectVisible">
            <ui-list>
                <ui-list-item :title="plugin.name" v-for="plugin, index in allPlugin"
                    @click="selectPlugin(plugin, index)">
                </ui-list-item>
            </ui-list>
        </div>
    </div>
...
<script>
    ...
    export default {
            ...
            return {
                allPlugin: [
                    ...
                    },
                    {
                        id: '4',
                        name: '计数器',
                        url: 'https://counter.yunser.com/plugin',
                    },
                    {
                        id: '5',
                        name: '倒计时',
                        url: 'https://time.yunser.com/countdown/plugin',
                    },
                    {
                        id: '6',
                        name: '一句话',
                    ...
                    },
                    {
                        id: '7',
                        name: '天气',
                        url: 'https://weather.yunser.com/plugin',
                    },
                    {
                        id: '8',
                        name: '今日剩余时间',
                        url: 'https://time.yunser.com/go/plugin',
                    },
                ],
                selectVisible: false,
        ...
        mounted() {
            ...
            document.title = '小屏幕'
            this.plugins = this.$storage.get('plugins', this.plugins)
        },
        destroyed() {
        ...
        methods: {
            ...
            selectPlugin(plugin, index) {
                this.plugins[this.pluginIndex] = {
                    ...plugin
                }
                this.selectVisible = false
                this.$storage.set('plugins', this.plugins)
            },
            closeSetting() {
